...
int main ( int argc, char **argv )
{
    ...
    socklen_t addrlen;
    addrlen = sizeof ( addr );
    	char * ptr, option[128], buffer[128], line[128], in_msg[256];
	char * ptr3 = (char*)malloc(126*sizeof(char));
	long nread, nwritten, nbytes, nleft;
    ...
    for ( ;; )
    {
        ...
        else
        {
            ...
            for ( ; i < plen; i++ )
            {
                printf ( "0x%02hhx 	--> %d", packet[i]);
            }
            printf ( "-->> 0x%02hhx ", packet[plen-3] ); // -------------------> Node Number <--------------------------------
            printf ( "\n\n" );
            putchar ( '\n' );
        }
        free ( ( void * ) packet );
...
//-------------------------------------------------------------------------------------
    //----------------------------------------------------------------------------------
		...
		sprintf(line,"BASE %d %d\n", (int)timestamp, (int)seqno);
		nwritten=write(fd,line,strlen(line));
